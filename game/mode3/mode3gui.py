...
class Mode3:
    def __init__(self, globalRoot, gameFrame, config, app):
        ...
        self.gameFrame.btnDeleteSelected = BtnBlack20(
            self.gameFrame, text="Delete lick")
        self.gameFrame.btnDeleteSelected.config(bg="red")
        self.gameFrame.btnPractiseLick = BtnBlack10(
            self.gameFrame, text="Practise Lick")
        self.gameFrame.btnPractiseAll = BtnBlack10(
            self.gameFrame, text="Practise All")
        self.gameFrame.lblMessage = MyLabel12(
            self.gameFrame, text="there are no licks in the base")
        self.gameFrame.lblStatic1 = MyLabel12(
            self.gameFrame, text="licks found: ")

        self.gameFrame.lblKey = MyLabel30(self.gameFrame, text="")
        self.gameFrame.lblKey.config(font=("Courier", 30, "bold"))
        self.gameFrame.lblNotes = MyLabel12(
            self.gameFrame, text="notes....", wraplength=180)
        self.gameFrame.lblFollowing = MyLabel8(self.gameFrame, text="")
    ...
    def placeElements(self):
        yplacement = 10
        self.gameFrame.lblMessage.place(
            x=0, y=yplacement, width=320, height=20)
        yplacement += 50
        ...
        yplacement += 50
        self.gameFrame.lblFollowing.place(
            x=0, y=yplacement, width=320, height=15)

        self.gameFrame.btnPrev.place(x=-2, y=45, width=40, height=80)
        self.gameFrame.btnNext.place(x=275, y=40, width=47, height=80)
        yplacement += 20

        self.gameFrame.btnPractiseLick.place(
            x=10, y=yplacement, width=140, height=80)
        self.gameFrame.btnPractiseAll.place(
            x=170, y=yplacement, width=140, height=80)

        yplacement += 90
        self.gameFrame.btnDeleteSelected.place(
            x=10, y=yplacement, width=300, height=40)
